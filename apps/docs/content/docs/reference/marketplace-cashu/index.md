---
title: "Marketplace Cashu Driver"
description: "Generated API reference for the Cashu marketplace driver."
full: true
---

# Marketplace Cashu Driver

## Namespaces

- [auction](/docs/reference/marketplace-cashu/Marketplace-Cashu-Driver/namespaces/auction/README)
- [escrow](/docs/reference/marketplace-cashu/Marketplace-Cashu-Driver/namespaces/escrow/README)

## Classes

- [CashuPaymentAmountLimitError](/docs/reference/marketplace-cashu/classes/CashuPaymentAmountLimitError)

## Type Aliases

- [CashuAmount](/docs/reference/marketplace-cashu/type-aliases/CashuAmount)
- [CashuAuctionPaymentPolicy](/docs/reference/marketplace-cashu/type-aliases/CashuAuctionPaymentPolicy)
- [CashuAuctionPolicy](/docs/reference/marketplace-cashu/type-aliases/CashuAuctionPolicy)
- [CashuAuctionPolicyOptions](/docs/reference/marketplace-cashu/type-aliases/CashuAuctionPolicyOptions)
- [CashuAuctionPolicyState](/docs/reference/marketplace-cashu/type-aliases/CashuAuctionPolicyState)
- [CashuEscrowOperation](/docs/reference/marketplace-cashu/type-aliases/CashuEscrowOperation)
- [CashuEscrowOperationQuery](/docs/reference/marketplace-cashu/type-aliases/CashuEscrowOperationQuery)
- [CashuEscrowOperationStatus](/docs/reference/marketplace-cashu/type-aliases/CashuEscrowOperationStatus)
- [CashuEscrowPaymentPolicy](/docs/reference/marketplace-cashu/type-aliases/CashuEscrowPaymentPolicy)
- [CashuEscrowPolicy](/docs/reference/marketplace-cashu/type-aliases/CashuEscrowPolicy)
- [CashuEscrowPolicyOptions](/docs/reference/marketplace-cashu/type-aliases/CashuEscrowPolicyOptions)
- [CashuEscrowPolicyState](/docs/reference/marketplace-cashu/type-aliases/CashuEscrowPolicyState)
- [CashuEscrowStorage](/docs/reference/marketplace-cashu/type-aliases/CashuEscrowStorage)
- [CashuMarketplacePolicyOptions](/docs/reference/marketplace-cashu/type-aliases/CashuMarketplacePolicyOptions)
- [CashuMintConfig](/docs/reference/marketplace-cashu/type-aliases/CashuMintConfig)
- [CashuPaymentAmountLimits](/docs/reference/marketplace-cashu/type-aliases/CashuPaymentAmountLimits)
- [CashuPaymentAsset](/docs/reference/marketplace-cashu/type-aliases/CashuPaymentAsset)
- [CashuPaymentPolicy](/docs/reference/marketplace-cashu/type-aliases/CashuPaymentPolicy)
- [GenericAuctionSettlementIntent](/docs/reference/marketplace-cashu/type-aliases/GenericAuctionSettlementIntent)
- [GenericAuctionSettlementResult](/docs/reference/marketplace-cashu/type-aliases/GenericAuctionSettlementResult)
- [GenericBolt11PaymentRequest](/docs/reference/marketplace-cashu/type-aliases/GenericBolt11PaymentRequest)
- [GenericPaymentIdentity](/docs/reference/marketplace-cashu/type-aliases/GenericPaymentIdentity)
- [GenericPaymentIntent](/docs/reference/marketplace-cashu/type-aliases/GenericPaymentIntent)
- [GenericPaymentProof](/docs/reference/marketplace-cashu/type-aliases/GenericPaymentProof)
- [GenericPaymentSweepInput](/docs/reference/marketplace-cashu/type-aliases/GenericPaymentSweepInput)
- [GenericPaymentSweepState](/docs/reference/marketplace-cashu/type-aliases/GenericPaymentSweepState)
- [GenericPaymentValidationRequest](/docs/reference/marketplace-cashu/type-aliases/GenericPaymentValidationRequest)
- [GenericPaymentValidationResult](/docs/reference/marketplace-cashu/type-aliases/GenericPaymentValidationResult)
- [GenericPolicyPaymentState](/docs/reference/marketplace-cashu/type-aliases/GenericPolicyPaymentState)
- [GenericSwapResumeContext](/docs/reference/marketplace-cashu/type-aliases/GenericSwapResumeContext)
- [GenericSwapResumeState](/docs/reference/marketplace-cashu/type-aliases/GenericSwapResumeState)

## Functions

- [createCashuAuctionPolicy](/docs/reference/marketplace-cashu/functions/createCashuAuctionPolicy)
- [createCashuEscrowPolicy](/docs/reference/marketplace-cashu/functions/createCashuEscrowPolicy)
